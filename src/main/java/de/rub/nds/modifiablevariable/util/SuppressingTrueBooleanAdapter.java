 ...
 * suppresses the {@code true} value when marshaling to XML. This means that boolean
 * fields with a value of {@code true} will be omitted from the XML output, making
 * the XML more concise when {@code true} is the default or common value.
 * 
 * <p>When unmarshaling XML, if an element is missing, this adapter will interpret
 * that as a {@code true} value.
 * 
 * <p>Usage example:
 * <pre>{@code
 ...
 * }</pre>
 * 
 * <p>With this adapter, in the example above, the "enabled" element will only
 * appear in the XML if its value is {@code false}. If it's {@code true} (the default),
